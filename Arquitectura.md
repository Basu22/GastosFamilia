...
- **Estructura de Visualización**:
  ...
  - **Por Categoría**: Reemplaza al antiguo filtro por tipo de movimiento. Muestra todas las categorías activas en el sistema en una grilla responsiva de **3 columnas por fila**. Calcula de forma dinámica la suma mensual de cada categoría a partir de los datos de `data.movimientos_mes` para el período consultado, incluyendo un elemento de fallback `"Sin Categoría"` para agrupar consumos que no cuenten con un tag del catálogo.
- **Detalle Dinámico y Acciones Integradas (ModalTarjetaDetalle)**:
  - Al abrir el modal detallado, se renderiza la lista de consumos. Si se visualiza por Tarjeta, agrupa los ítems por tipo; si se visualiza por Categoría, los lista de manera plana incluyendo una etiqueta de procedencia de fondos y el medio de pago utilizado.
  - Cuenta con un botón de adición rápida `+` (en el encabezado de grupo en tarjetas o en el tope del modal en categorías/movimientos) que abre el formulario `InlineCreateForm` para registrar nuevos consumos al vuelo sin abandonar el modal.
...
- **Bloqueo y Seguridad en Carga Rápida (InlineCreateForm)**:
  ...
  - Al cargar consumos desde el modal de una categoría, el selector **"Categoría"** se preselecciona y bloquea de manera análoga (`disabled` + `pointer-events-none`) para garantizar la persistencia del consumo dentro de la categoría activa.
