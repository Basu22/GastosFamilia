...
import { createMovimiento, previewMovimiento } from '../api/movimientos';
import { formatARS } from '../utils/format';
import { CreditCard, Info, Save, X } from 'lucide-react';
...
export default function NuevoGasto() {
  ...
  return (
    <main id="page-nuevo-gasto" className="max-w-2xl mx-auto space-y-8">
      <header id="header-nuevo-gasto" className="flex items-center gap-3">
        ...
        <div>
          <h1 id="title-nuevo-gasto" className="text-2xl font-bold text-gray-900 dark:text-neutral-100">Nuevo Gasto</h1>
          <p className="text-gray-500 dark:text-neutral-400 text-sm">Registrá una compra nueva</p>
        </div>
      </header>

      <form id="form-nuevo-gasto" onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <section id="section-form-inputs" className="bg-white dark:bg-neutral-900 rounded-3xl border border-gray-100 dark:border-neutral-800 shadow-sm p-4 lg:p-8 space-y-6 transition-colors">
          <div className="space-y-2">
            ...
            {errors.tarjeta_id && <p className="text-xs font-medium text-red-500">{errors.tarjeta_id.message}</p>}
          </div>
          ...
          <div className="space-y-2">
            ...
            />
            {errors.descripcion && <p className="text-xs font-medium text-red-500">{errors.descripcion.message}</p>}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              />
              {errors.monto_total && <p className="text-xs font-medium text-red-500">{errors.monto_total.message}</p>}
            </div>
          </div>
        </section>
        ...
        <section id="section-fecha" className="flex flex-col gap-2">
          ...
          />
          {errors.fecha_primera_cuota && <p id="error-fecha" className="text-xs font-medium text-red-500">{errors.fecha_primera_cuota.message}</p>}
        </section>
        ...
        <section id="section-cuotas" className="flex flex-col gap-2">
          ...
          {/* Input oculto para registrar con react-hook-form */}
          <input type="hidden" {...register('cuotas', { valueAsNumber: true })} />
        </section>

        {/* Preview Panel */}
        {previewData && (
          <article id="panel-preview" className="bg-blue-50 dark:bg-blue-950/40 rounded-2xl p-4 border border-blue-100 dark:border-blue-900/50 flex items-start gap-3 transition-colors">
            ...
            <div>
              ...
              <p className="text-xs text-blue-700 dark:text-blue-400/80 mt-1">
                La última cuota se cobrará el {new Date(previewData.fecha_ultima_cuota).toLocaleDateString('es-AR')}.
              </p>
            </div>
          </article>
        )}
        ...
        <button 
          ...
          className="w-full flex items-center justify-center gap-2 py-4 rounded-xl font-bold text-white bg-blue-600 hover:bg-blue-700 active:scale-[0.98] transition-all disabled:opacity-70 disabled:active:scale-100 shadow-lg shadow-blue-200"
        >
          {mutation.isPending ? 'Guardando...' : <><Save size={20} /> Guardar Gasto</>}
        </button>
